...
class EnhancedUTRP:
    ...
    def is_feasible(self, route_set: List[List[int]]) -> bool:
        """Проверка feasibility с проверкой связности маршрутов"""
                    ...
                    if not found_bypass:
                        ...
                        if i == 0:
                            ...
                            if (self.m_min <= len(new_route1) <= self.m_max and 
                                ...
                                # Если разделение невозможно, удаляем разрыв
                                del route[i+1]
                else:
                    i += 1
        
        # Удаляем пустые маршруты и добавляем новые если нужно
        repaired = [route for route in repaired if len(route) >= self.m_min]
        
        while len(repaired) < self.r:
