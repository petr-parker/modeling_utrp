...
class EnhancedUTRP:
    ...
    def __init__(self, weight_matrix: np.ndarray, demand_matrix: np.ndarray,
                 ...
                 default_frequency: float = 5.0, transfer_penalty: float = 2.0,
                 min_frequency: float = 1.0, max_frequency: float = 20.0):
        ...
        # Параметры частот автобусов
        self.default_frequency = default_frequency  # рейсов в час по умолчанию
        # фиксированный штраф за пересадку (мин)
        self.transfer_penalty = transfer_penalty
        # минимальная частота (рейсов/час)
        self.min_frequency = min_frequency
        # максимальная частота (рейсов/час)
    ...
    def yens_k_shortest_paths(self, source: int, target: int, K: int) -> List[List[int]]:
        ...
        for k in range(1, K):
            ...
            for i in range(len(prev_path) - 1):
                ...
                spur_path = self._reconstruct_path_static(
                    W_temp, spur_node, target)
    ...
    def build_accessibility_graph(self, route_set: List[List[int]]) -> np.ndarray:
        ...
        # Используем фиксированный штраф transfer_penalty + среднее время ожидания
        # При отсутствии частот используем default_frequency
        # среднее ожидание в минутах
        default_waiting_time = 60 / (2 * self.default_frequency)
        transfer_cost = self.transfer_penalty + default_waiting_time
                ...
                for j in range(self.n):
                    if (graph[i][k] < float('inf') and
                        graph[k][j] < float('inf') and
                            graph[i][k] + graph[k][j] + transfer_cost < graph[i][j]):
                        graph[i][j] = graph[i][k] + graph[k][j] + transfer_cost

        return graph

    def build_accessibility_graph_with_frequencies(
        self, route_set: List[List[int]], frequencies: List[float]
    ) -> np.ndarray:
        """
        Построение графа доступности с учётом частот маршрутов.

        При пересадке на маршрут r стоимость = transfer_penalty + waiting_time[r]
        где waiting_time[r] = 60 / (2 * frequencies[r])

        Args:
            route_set: список маршрутов
            frequencies: частоты для каждого маршрута (рейсов/час)
        """
        if len(frequencies) != len(route_set):
            raise ValueError(
                "Количество частот должно соответствовать количеству маршрутов")

        # Время ожидания для каждого маршрута (в минутах)
        waiting_times = [60 / (2 * f) if f > 0 else 9999 for f in frequencies]

        graph = np.full((self.n, self.n), float('inf'))
        np.fill_diagonal(graph, 0)

        # Создаём mapping: вершина -> список (route_idx, position_in_route)
        vertex_to_routes: Dict[int, List[Tuple[int, int]]] = defaultdict(list)
        for route_idx, route in enumerate(route_set):
            for pos, vertex in enumerate(route):
                vertex_to_routes[vertex].append((route_idx, pos))

        # Прямые соединения внутри маршрутов (без ожидания)
        for route_idx, route in enumerate(route_set):
            for i in range(len(route)):
                for j in range(i, len(route)):
                    # Проверяем связность подпути
                    subpath_connected = True
                    time = 0
                    for k in range(i, j):
                        weight = self.W[route[k]][route[k+1]]
        ...
        # Пересадки с учётом частот
        # Используем модифицированный Флойд-Уоршелл
        # При пересадке в вершине k на маршрут target_route:
        # cost = transfer_penalty + waiting_times[target_route]

        # Для корректного учёта пересадок нужно знать, на какой маршрут пересаживаемся
        # Упрощённый подход: берём минимальное время ожидания среди маршрутов, проходящих через k
        for k in range(self.n):
            # Находим минимальное время ожидания среди маршрутов через вершину k
            routes_through_k = vertex_to_routes[k]
            if routes_through_k:
                min_waiting = min(waiting_times[route_idx]
                                  for route_idx, _ in routes_through_k)
            else:
                min_waiting = 60 / (2 * self.default_frequency)

            transfer_cost = self.transfer_penalty + min_waiting
    ...
    def calculate_att_with_frequencies(
        self, route_set: List[List[int]], frequencies: List[float]
    ) -> float:
        """
        Расчёт ATT с учётом частот маршрутов.
        ...
        accessibility_graph = self.build_accessibility_graph_with_frequencies(
            route_set, frequencies
        )
    ...
    def calculate_operating_cost(
        ...
        Operating Cost = Sum(2 * Length_r * Frequency_r)
        ...
        for route_idx, route in enumerate(route_set):
            ...
            for i in range(len(route) - 1):
                ...
                if weight < float('inf'):
                    route_length += weight
            # 2 * length (туда и обратно) * frequency
            total_cost += 2 * route_length * frequencies[route_idx]
        return total_cost
    ...
    def optimize_frequencies(
        ...
        w_att: float = 1.0,
        w_cost: float = 0.01,
        initial_frequencies: Optional[List[float]] = None
    ) -> Tuple[List[float], float, float]:
        """
        Оптимизация частот маршрутов для минимизации комбинированной целевой функции.

        Minimize: Z = w_att * ATT + w_cost * Operating_Cost

        Args:
            route_set: список маршрутов
            w_att: вес для ATT (время пассажиров)
            w_cost: вес для операционных затрат
            initial_frequencies: начальные частоты (если None, используется default_frequency)

        Returns:
            (optimal_frequencies, optimal_att, optimal_cost)
        """
        num_routes = len(route_set)

        # Начальное приближение
        if initial_frequencies is None:
            x0 = [self.default_frequency] * num_routes
        else:
            x0 = list(initial_frequencies)

        # Границы частот
        bounds = [(self.min_frequency, self.max_frequency)
                  for _ in range(num_routes)]

        def objective(freqs):
            att = self.calculate_att_with_frequencies(route_set, freqs)
            cost = self.calculate_operating_cost(route_set, freqs)
            return w_att * att + w_cost * cost
        ...
        optimal_frequencies = list(result.x)
        optimal_att = self.calculate_att_with_frequencies(
            route_set, optimal_frequencies)
        optimal_cost = self.calculate_operating_cost(
            route_set, optimal_frequencies)

        return optimal_frequencies, optimal_att, optimal_cost

    def evaluate_with_frequency_optimization(
        self, route_set: List[List[int]], w_att: float = 1.0, w_cost: float = 0.01
    ) -> Dict[str, Any]:
        """
        Полная оценка решения с оптимизацией частот.

        Returns:
            Словарь с результатами:
            - 'frequencies': оптимальные частоты
            - 'att': ATT с оптимальными частотами
            - 'operating_cost': операционные затраты
            - 'att_baseline': ATT с дефолтными частотами
            - 'improvement': улучшение ATT в процентах
        """
        # Базовый ATT (с дефолтными частотами)
        baseline_frequencies = [self.default_frequency] * len(route_set)
        att_baseline = self.calculate_att_with_frequencies(
            route_set, baseline_frequencies)
        cost_baseline = self.calculate_operating_cost(
            route_set, baseline_frequencies)

        # Оптимизация
        opt_freq, opt_att, opt_cost = self.optimize_frequencies(
            route_set, w_att, w_cost)

        improvement = (att_baseline - opt_att) / att_baseline * \
            100 if att_baseline > 0 else 0

        return {
            'frequencies': opt_freq,
            'att': opt_att,
            'operating_cost': opt_cost,
            'att_baseline': att_baseline,
            'cost_baseline': cost_baseline,
            'att_improvement_percent': improvement,
            'cost_change_percent': (opt_cost - cost_baseline) / cost_baseline * 100 if cost_baseline > 0 else 0
        }
            ...
            for route_idx, route in enumerate(route_set):
                ...
                for i in range(len(route) - 1):
                    if (self.W[route[i]][vertex] < float('inf') and
                        ...
                        new_cost = self.W[route[i]][vertex] + \
                            self.W[vertex][route[i+1]]
    ...
    def is_connected(self, route_set: List[List[int]]) -> bool:
        ...
        # Допускаем 10% изолированных
        return len(visited) >= len(all_vertices) * 0.9
    ...
    def cost_based_trim_operator(self, route_set: List[List[int]]) -> List[List[int]]:
        ...
        for position, vertex_idx in [('first', 0), ('last', -1)]:
            ...
            if self.is_route_feasible(test_route):
                ...
                if vertex_idx == 0:
                    cost_saving = self.W[route[0]][route[1]] if len(
                        route) > 1 else 0
                else:
                    cost_saving = self.W[route[-2]
                                         ][route[-1]] if len(route) > 1 else 0
    ...
    def calculate_route_demand(self, route: List[int]) -> float:
        ...
        for i in range(len(route)):
            for j in range(i, len(route)):
                total_demand += self.D[route[i]][route[j]
                                                 ] + self.D[route[j]][route[i]]
        return total_demand
    ...
    def dbmosa(self, max_iterations: int = 1000, initial_temp: float = 1.0,
        ...
        for iteration in range(max_iterations):
            ...
            new_solution = self.apply_operator_with_repair(
                operator, current_solution)
            ...
            if delta_energy <= 0 or random.random() < np.exp(-delta_energy / temperature):
                ...
                if new_dominated == 0:
                    ...
                    archive = [sol for sol in archive if not self.dominates(
                        new_solution, sol)]
        ...
        best_solution = min(archive, key=lambda x: (
            self.calculate_att(x), self.calculate_trt(x)))
    ...
    def nsga_ii(self, population_size: int = 50, generations: int = 100,
        ...
        population = self.generate_initial_solutions(
    ...
    def crossover(self, parent1: List[List[int]], parent2: List[List[int]]) -> List[List[int]]:
        ...
        if remaining_slots > 0:
            ...
            available_routes = [
                route for route in all_routes if route not in child]
